...
class AdminPelatihan extends CI_Controller {
    ...
    public function add(){
       if($this->mdl_admin->logged_id()){
            ...
            if($this->form_validation->run()==FALSE){
                ...
                $this->mdl_admin->addData('mou_pelatihan',$data);

                redirect("Adminpelatihan");
                }
        }
        else{ redirect("login"); } 
    }
            ...
            if($this->form_validation->run()==FALSE){
                ...
                if($_FILES['file']['name'] != '') {
                    if(!$this->upload->do_upload('file')) {
                        ...
                        redirect("adminpelatihan/edit/$id");
                    } else {
                        $file = $this->upload->data('file_name');
                    }
                } else {
                ...
                $where = array('id' => $id);
                $this->mdl_admin->updateData($where,$data,'mou_pelatihan');
                redirect("Adminpelatihan");
                }
        }
    ...
    public function del($id){
        $where = array('id' => $id);
        $this->mdl_pelamar->hapusdata('mou_pelatihan',$where);
        redirect("Adminpelatihan");
    }
    ...
    if(isset($_FILES["file"]["name"]))
        {
            ...
            $this->mdl_admin->impor('mou_pelatihan',$data);
            redirect('Adminpelatihan');
        }        
    }
}
